...
export const getLead = async (req, res) => {
  ...
  if (req.query.first_name)
    filter.first_name = { $regex: req.query.first_name, $options: "i" };
  if (req.query.last_name)
    filter.last_name = { $regex: req.query.last_name, $options: "i" };
  ...
  if (req.query.lead_value || req.query.minValue || req.query.maxValue) {
    ...
    if (req.query.lead_value)
      filter.lead_value.$eq = Number(req.query.lead_value);
  ...
  if (req.query.created_after || req.query.created_before) {
    ...
    if (req.query.created_before)
      filter.createdAt.$lte = new Date(req.query.created_before);
  }

  if (req.query.updated_after || req.query.updated_before) {
    filter.updatedAt = {};
    if (req.query.updated_after)
      filter.updatedAt.$gte = new Date(req.query.updated_after);
    if (req.query.updated_before)
      filter.updatedAt.$lte = new Date(req.query.updated_before);
  }
...
export const updateLead = async (req, res) => {
  ...
  //  Update lead in DB
  const updatedLead = await Lead.findByIdAndUpdate(leadId, updatedInfo, {
    new: true,
  });
